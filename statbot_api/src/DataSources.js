...
var DataSources = {
  riot: {
    ...
    // todo: rewrite using irelia
    // todo: figure out nested data source needs
    ranked_stats: DataSource('riot_ranked_stats', ONE_HOUR, Cache,
      function (opts, done) {
        ...
        var summoner_id_promise = RIOT_SUMMONER_ID.get({
          summoner_name: summoner_name,
          region: region
        }).then(function (id) {
          // now actually go grab the ranked stats
          console.log("--ID: #{id}")
          LoL.statsRanked(id, region, function (ranked_stats) {
