...
// TODO: replace this with a data source
// Expose a friendly way to convert champion ids to names
  ...
  riot: {
    ...
    summoner_id: DataSource('riot_summoner_id', ONE_WEEK, Cache,
      function (opts, done) {
        ...
        var region = opts.region || "na"
        Irelia.getSummonerByName(region, summoner_name, function (err, data) {
          if (err) {
            done(err);
            return;
          }
    ...
    ranked_stats: DataSource('riot_ranked_stats', ONE_HOUR, Cache,
      function (opts, done) {
        var summoner_name = opts.summoner_name;
        var region = opts.region || "na";
        var season = opts.season || "SEASON2015"
        ...
        var summoner_id_promise = DataSources.riot.summoner_id.get({
          ...
          // console.log("ID: ", id)
          Irelia.getRankedStatsBySummonerId(region, id, season, function (err, ranked_stats) {
            if (err) {
              done(err);
              return;
            }
